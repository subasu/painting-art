    ...
    <div class="row">
                    ...
                    <table style="direction:rtl;text-align: center" id="example"
                        ...
                        <form id="editForm">
                            ...
                            <tr class="unit">
                                ...
                                <td class="col-md-6 "><input  class="form-control" style="width: 100%;" id="title" name="title" value="{{$model[0]->title}}"></td>
                                <td><button id="edit" type="button" class="btn btn-warning col-md-9 col-md-offset-1">ویرایش</button></td>
                                @if($model[0]->active == 1)
                                    <td><a id="active" content="{{$model[0]->active}}" name="{{$model[0]->id}}" type="button"  data-content="غیر فعال" class="btn btn-danger" >غیر فعال</a></td>
                                @endif
                                @if($model[0]->active == 0)
                                    <td><a id="active" content="{{$model[0]->active}}" name="{{$model[0]->id}}" type="button"  data-content="فعال" class="btn btn-success"> فعال</a></td>
                                @endif
                                <input type="hidden" value="{{$model[0]->id}}" id="id" name="id">
        ...
        <script>
            $(document).on('click','#active',function () {
                ...
                //alert(title);
                swal({
                        title:   " آیا در نظر دارید وضعیت  این حالت را " +"(( "+ title +" ))"+  "  نمایید؟ ",
                        text: "",
                        type: "info",
                        showCancelButton: true,
                        confirmButtonColor: "	#5cb85c",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            ({
                                cache: false,
                                url: "{{url('admin/enableOrDisableModel')}}",
                                type: "post",
                                data: {'_token': token, 'active': active, 'modelId': modelId},
                                dataType: "JSON",
                                success: function (response) {
                                    if (response.code == 1) {
                                        swal({
