            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="pictureForm" enctype="multipart/form-data">
                        {{csrf_field()}}
                        @if($gallery->image_src != null)
                            <img class="image" id="editable"
                                 style="height: 100px; width: 100px; margin-left: 80%;"
                                 src="{{url('public/dashboard/Gallery')}}/{{$gallery->image_src}}">
                        @endif
                        @if($gallery->image_src == null)
                        ...
                        <input type="hidden" name="sliderId" value="{{$gallery->id}}">
                    </form>
                </div>
                <div class="modal-footer" >
                    <button type="button" class="btn btn-dark col-md-5 col-md-offset-1" data-dismiss="modal">بستن</button>
                    <button type="button" content="{{$gallery->id}}" id="editSliderPicture" class="btn btn-warning col-md-5" >ویرایش</button>
                </div>
            </div>
    ...
    <div class="row">
                    ...
                    <table style="direction:rtl;text-align: center" id="example"
                        ...
                        <form id="editForm">
                            ...
                            <tr class="unit">
                                ...
                                <td class="col-md-4 "><input  class="form-control" style="width: 100%;" id="title" name="title" value="{{$gallery->title}}"></td>
                                ...
                                {{--@if($gallery->active == 1)--}}
                                    {{--<td><a id="active" content="{{$gallery->active}}" name="{{$gallery->id}}" type="button"  data-content="غیر فعال" class="btn btn-danger" >غیر فعال</a></td>--}}
                                {{--@endif--}}
                                {{--@if($gallery->active == 0)--}}
                                    {{--<td><a id="active" content="{{$gallery->active}}" name="{{$slider->id}}" type="button"  data-content="فعال" class="btn btn-success"> فعال</a></td>--}}
                                {{--@endif--}}
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            ({
                                cache: false,
                                url: "{{url('admin/editGalleryTitle')}}",
        ...
        <script>
            $(document).on('click','#editSliderPicture',function(){
                ...
                ({
                    cache        : false,
                    url          : "{{url('admin/editGalleryPicture')}}",
                    type         : "post",
                    data         : formData,
                    dataType     : "JSON",
                    processData  : false,
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            ({
                                cache: false,
                                url: "{{url('admin/enableOrDisableGallery')}}",
                                type: "post",
